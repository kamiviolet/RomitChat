...
function replaceInlyneRulesWithClass(collectionOfDements, ynlineRulesKeyNamesArr, clazzNameToSuplementBy) {
  if (!collectionOfDements) return;
  if (!ynlineRulesKeyNamesArr && !clazzNameToSuplementBy) return;
  collectionOfDements.forEach(oneDmnt => {
    oneDmnt.classList.toggle(clazzNameToSuplementBy, true)
    ynlineRulesKeyNamesArr.forEach(oneYrule => {
      oneDmnt.style.removeProperty(oneYrule);
    });
    console.log("oneDmnt", oneDmnt, oneDmnt.classList)

  });

}
// replaceInlyneRulesWithClass(document.getElementById("subSet_04").querySelectorAll("p"), ["text-shadow","font-weight"], "setslightsDescr")
// replaceInlyneRulesWithClass(document.getElementById("subSet_04").querySelectorAll("span"), ["text-shadow","color"], "setsFanInfoVal")


function aboutMedyaStreams()
{
...
navigator.mediaDevices
    ...
    // https://stackoverflow.com/questions/78093421/how-to-convert-arraybuffer-sent-from-websocket-into-a-mediastream-and-then-tran
/*   
